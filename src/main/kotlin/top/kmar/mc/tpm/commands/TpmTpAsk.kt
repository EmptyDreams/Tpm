...
object TpmTpAsk {
    ...
    fun registry(dispatcher: CommandDispatcher<CommandSourceStack>) {
        ...
        dispatcher.register(
            literal("tpaccept").executes { context ->
                ...
                if (list.isEmpty()) {
                    player.sendTpmMessage(TpmCommand.errorText("没有可接受的传送请求"))
                } else {
                    TpRequestManager.acceptRequests(list)
                    player.sendTpmMessage(Component.literal("已接受所有传送请求"))
                }
                1
            }.then(
                argument("player", EntityArgument.player())
        ...
        dispatcher.register(
            literal("tpreject").executes { context ->
                ...
                if (list.isEmpty()) {
                    player.sendTpmMessage(TpmCommand.errorText("没有可拒绝的传送请求"))
                } else {
                    TpRequestManager.rejectRequests(list)
                    player.sendTpmMessage(Component.literal("已拒绝所有传送请求"))
                }
                1
            }.then(
                argument("player", EntityArgument.player())
        ...
        dispatcher.register(
            literal("tpcancel").executes { context ->
                ...
                if (request == null) {
                    player.sendTpmMessage(TpmCommand.errorText("没有可取消的传送请求"))
                } else {
                    request.cancel()
    ...
    private fun executeTphereCommand(context: CommandContext<CommandSourceStack>, force: Boolean): Int {
        ...
        if (player == targetPlayer) {
            player.sendTpmMessage(TpmCommand.errorText("传送自己听起来像是未来的科技，目前我们的魔法还不足以让你和自己玩捉迷藏。"))
            return 0
        }
        if (player.tpmData.autoReject) {
            targetPlayer.sendTpmMessage(TpmCommand.grayText("您的传送请求已被自动拒绝"))
            return 1
        }
        val success = TpRequestManager.appendRequest(
            ...
            rejectEvent = {
                targetPlayer.sendTpmMessage(Component.empty().append(player.name).append(" 拒绝了您的传送请求"))
            }
        ), force)
        if (!success) {
            player.sendTpmMessage(
    ...
    private fun executeTpaCommand(context: CommandContext<CommandSourceStack>, force: Boolean): Int {
        ...
        if (targetPlayerConfig.autoReject) {
            player.sendTpmMessage(TpmCommand.grayText("您的传送请求已被自动拒绝"))
            return 1
        }
        ...
        val success = TpRequestManager.appendRequest(TpRequestManager.TpRequest(
            ...
            rejectEvent = {
                player.sendTpmMessage(Component.empty().append(targetPlayer.name).append(" 拒绝了您的传送请求"))
            },
            cancelEvent = {
                targetPlayer.sendTpmMessage(
